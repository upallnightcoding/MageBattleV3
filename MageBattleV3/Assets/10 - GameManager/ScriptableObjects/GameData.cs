...
public class GameData : ScriptableObject
{
    ...
    public MazeGenerator GetMaze() => (maze);
    ...
    public void CreatePath(Framework framework, MazeCell mazeCell, Vector3 position, uint columns, uint walls)
    {
        ...
        GameObject path = framework
            ...
            .Assemble(northWall, NORTH_ANCHOR, 90.0f)
            .Assemble(southWall, SOUTH_ANCHOR, 90.0f)
            .Assemble(eastWall, EAST_ANCHOR)
            .Assemble(westWall, WEST_ANCHOR)
            .Assemble(buildingColumnPreFab, NORTH_EAST_ANCHOR, 0.0f, (columns & NE) > 0)
            .Assemble(buildingColumnPreFab, NORTH_WEST_ANCHOR, 0.0f, (columns & NW) > 0)
            .Assemble(buildingColumnPreFab, SOUTH_EAST_ANCHOR, 0.0f, (columns & SE) > 0)
            .Assemble(buildingColumnPreFab, SOUTH_WEST_ANCHOR, 0.0f, (columns & SW) > 0)
            .Position(position)
            .Build();
    }
